...
def build_hybrid_prompt(question: str, structured_summary: str, semantic_rows: pd.DataFrame) -> str:
    """
    ...
    if semantic_rows is None or semantic_rows.empty:
        ...
        for _, row in limited.iterrows():
            bullets.append(
                f"• Float {row.get('float_id', 'Unknown')} | {row.get('date', 'Unknown')} | "
                f"{row.get('depth_m', 0):.0f}m | {row.get('temp_c', 0):.1f}°C | "
                f"{row.get('salinity', 0):.2f} PSU"
            )
